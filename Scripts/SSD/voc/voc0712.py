...
from Scripts.SSD.voc import *
from Scripts.SSD.utils import binvox_rw
import csv
import random
import os
...
VOC_CLASSES = ('O ring', 'Through hole', 'Blind hole',
               ...
               '6-sides passage', '6-sides pocket', 'COUNTER')
...
def rotate_sample24(sample):
    ...
    #
    #    if strategy <= 2:
    #        sample = np.flip(sample,strategy).copy()
...
def achieve_legal_model(list_IDs):

    filename = list_IDs[random.randint(0, len(list_IDs) - 1)]
    filename = "TrainigData/all_BINVOX/" + filename
...
def achieve_random_model(list_IDs,counter):
    # num_features = 2
    ...
    for i in range(num_features):
        label, model, filename = achieve_legal_model(list_IDs)
    ...
    #BinvoxSaver.write(ret_model, str(counter) + ".binvox")
